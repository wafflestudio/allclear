...
describe('getManagedClubUpdateErrorContent', () => {
	it.each([
		...
		expect(getManagedClubUpdateErrorContent(createAxiosError(status)).title).toBe(title)
	})

	it('응답 상태를 알 수 없으면 재시도 안내를 반환한다', () => {
		expect(getManagedClubUpdateErrorContent(new Error('network error'))).toEqual({
