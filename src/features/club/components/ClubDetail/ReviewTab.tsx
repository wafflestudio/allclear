...
import type { CategoryDetail } from "@/shared/constants/category";
...
const ReviewTab = ({
	...
	const sortedKeywords = [...club.reviewKeywords].sort(
		...
		<View style={styles.tabSection}>
			<BackgroundCard>
				...
				{club.totalReviews > 0 && (
					...
					>{`현재까지 ${club.totalReviews}명이 참여했어요`}</Text>
				)}
			...
			<Pressable onPress={onWriteReview}>
				{({ pressed }) => (
					<BackgroundCard
						style={[styles.ctaCard, pressed && styles.ctaCardPressed]}
					>
						<View style={styles.ctaRow}>
							<View>
								<Text style={styles.ctaTitle}>동아리 활동 후기 남기기</Text>
								<Text
									style={styles.ctaMeta}
								>{`현재까지 ${club.totalReviews}명이 참여했어요`}</Text>
							</View>
							<Icon
								name="chevron-right"
