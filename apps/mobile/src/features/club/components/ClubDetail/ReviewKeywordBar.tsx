...
const ReviewKeywordBar = ({ keyword, totalReviews, themeColor }: Props) => {
	const ratio =
		totalReviews > 0
			? Math.min(Math.max(keyword.totalUpvotes / totalReviews, 0), 1)
			: 0;
		...
		<View style={[styles.bar, { borderColor: themeColor }]}>
			...
			<View style={styles.content}>
				<View style={styles.left}>
					<Text style={[styles.icon, typography.bodySMedium]}>
						{keyword.iconUri?.trim()}
					</Text>
					<Text
						style={[styles.title, typography.bodySMedium]}
						numberOfLines={1}
					>
						{keyword.title}
					</Text>
				</View>
				<Text style={[styles.count, typography.bodySMedium]}>
