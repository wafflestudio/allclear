...
const SearchFilterBar = ({ filters, onChange, onPressFilter }: Props) => {
	...
	const clubTypeSelection =
		normalizedFilters.affiliation_types.length === 0
			...
			: {
					...
					values: normalizedFilters.affiliation_types,
				};

	const handleChangeAffiliationType = (
		...
		onChange({
			...normalizedFilters,
			affiliation_types:
				selection.values as ClubSearchFilters["affiliation_types"],
		});
	};
	...
	const handleToggleRecruiting = () => {
		onChange({
			...normalizedFilters,
			is_recruiting:
				normalizedFilters.is_recruiting === "true" ? undefined : "true",
		});
	};
