...
const useAutoScroll = <T>(itemCount: number) => {
	...
	const updateMaxOffset = () => {
		maxOffset.value = Math.max(
			latestContentWidth.current - latestListWidth.current,
			0,
		);
	};
	...
	useFocusEffect(
		// biome-ignore lint/correctness/useExhaustiveDependencies: Reanimated shared values and refs are stable; this reset should run only on focus.
		useCallback(() => {
			offset.value = 0;
			isInteracting.value = false;
			if (resumeTimeoutRef.current) {
