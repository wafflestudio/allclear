...
export type MinDurationStepValue =
	(typeof MIN_DURATION_OPTIONS)[number]["value"];
...
export const useMinDurationToggle = ({
	value,
	onChange,
}: UseMinDurationToggleOptions) => {
		...
		(durationValue: MinDurationStepValue) => {
			const nextSelectedValues = selectedValues.includes(durationValue)
				? selectedValues.filter(
						(selectedValue) => selectedValue !== durationValue,
					)
	...
	const handleLabelsContainerLayout = useCallback(
	...
	const lastLabelWidth =
		labelWidths[MIN_DURATION_OPTIONS[MIN_DURATION_OPTION_COUNT - 1].value] ?? 0;
	const trackStart = firstLabelWidth / 2;
	const trackEnd = Math.max(
		labelsContainerWidth - lastLabelWidth / 2,
		trackStart,
	);
	...
	const stepCenters = useMemo(
		() =>
			MIN_DURATION_OPTIONS.map(
				(_, index) =>
					trackStart + (trackWidth * index) / (MIN_DURATION_OPTION_COUNT - 1),
			),
