...
const TermsAgreementModal = ({
	...
	const isAllChecked =
		allTermUuids.length > 0 && checkedTermUuids.length === allTermUuids.length;
	const isAgreeButtonEnabled =
		terms.length > 0 &&
		mandatoryTermUuids.every((uuid) => checkedTermUuids.includes(uuid));

	const handleToggleTerm = (termUuid: Term["uuid"]) => {
		setCheckedTermUuids((prev) =>
			prev.includes(termUuid)
				? prev.filter((uuid) => uuid !== termUuid)
				: [...prev, termUuid],
		);
	};
								...
								{terms.map((term, index) => {
									...
									return (
										<View
											...
											style={[
												styles.termRow,
												index < terms.length - 1 && styles.termRowBorder,
											]}
										>
											<Checkbox
