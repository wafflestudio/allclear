import { areValidSnsUrls, getSnsRequestFields, normalizeSnsUrls } from './validation'

describe('SNS URL validation', () => {
	it('normalizes every SNS URL', () => {
		expect(normalizeSnsUrls(['instagram.com/club', ' https://youtube.com/@club '])).toEqual([
	...
	it('accepts one to three valid SNS URLs', () => {
		...
		expect(areValidSnsUrls(['instagram.com/club', 'youtube.com/@club', 'facebook.com/club'])).toBe(
			true,
		)
	})
