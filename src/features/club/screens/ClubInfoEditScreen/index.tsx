...
const getClubDepartment = (club: ManagedClubDetail): string => {
	...
	return club.affiliationType && club.affiliationType !== "소속동아리"
...
const SuccessModal = memo(
	({ visible, title, onConfirm }: SuccessModalProps) => (
		<Modal
			visible={visible}
			transparent
			animationType="fade"
...
const ClubActivityPeriodSection = memo(
	({
		...
		onDecrement,
		onIncrement,
	}: ClubActivityPeriodSectionProps) => {
		return (
			<View style={styles.fieldWrapper}>
				...
				<View style={styles.periodRow}>
					<View style={styles.modeButtons}>
						<Pressable
							style={[
								styles.typeButton,
								mode === "none" && styles.typeButtonSelected,
							]}
						...
						>
							<Text
								style={[
									styles.typeButtonText,
									mode === "none" && styles.typeButtonTextSelected,
								]}
							>
								없음
							</Text>
						...
						<Pressable
							style={[
								styles.typeButton,
								mode === "number" && styles.typeButtonSelected,
							]}
						...
						>
							<Text
								style={[
									styles.typeButtonText,
									mode === "number" && styles.typeButtonTextSelected,
								]}
							>
								있음
							</Text>
			...
			<View style={styles.fieldWrapper}>
				...
				<View style={styles.buttonGroup}>
					<Pressable
						style={[
							styles.typeButton,
							hasDongbang && styles.typeButtonSelected,
						]}
					...
					>
						<Text
							style={[
								styles.typeButtonText,
								hasDongbang && styles.typeButtonTextSelected,
							]}
						>
							보유
						</Text>
...
const ClubInfoEditScreen = () => {
	...
	const setFormField = useCallback(
		<K extends keyof ClubInfoEditFormData>(
			key: K,
	...
	const handleCloseDepartmentDropdown = useCallback(
		() => setShowDepartmentDropdown(false),
		[],
	);
	...
	const handleActivityImagesChange = useCallback(
		(activityImages: EditableImage[]) =>
			setFormField("activityImages", activityImages),
		[setFormField],
	);
	const handleDescriptionChange = useCallback(
		(clubDescription: string) =>
			setFormField("clubDescription", clubDescription),
		[setFormField],
	);
	...
	const isSNSComplete =
		JSON.stringify(currentComparable.snsUrls) ===
	...
	const updateRequest = useMemo(() => {
		...
		return buildUpdateManagedClubRequest(
			clubId,
			currentComparable,
			initialComparable,
		);
	...
	const activityImagesChanged =
		...
		JSON.stringify(retainedActivityImageUrls) !==
			JSON.stringify(initialActivityImageUrls);
	const hasChanges = !!imageFile || !!updateRequest || activityImagesChanged;
	const hasAnyChanges =
		hasChanges || managerData !== undefined || isResubmission;
