...
import type { ListSavedClubsResponse } from "@/repositories/club";
import { useProfile } from "@/shared/contexts/profileContext";
import { serviceContext } from "@/shared/contexts/serviceContext";
import useRequireLogin from "@/shared/hooks/useRequireLogin";

const selectSavedIds = (data: ListSavedClubsResponse) =>
	new Set(data.clubs.map((c) => c.uuid)); // 모듈 레벨에서 선언하여 매번 새로 생성하지 않음
...
const useSaveClub = (club: Club | undefined) => {
	...
	useEffect(() => {
		if (!hasPendingRef.current) {
			serverIsSavedRef.current = club
				? (savedIds?.has(club.uuid) ?? false)
				: false;
		}
	}, [savedIds, club]);
			...
			queryClient.setQueriesData(
				...
				(old: { clubs: Club[]; totalSize: number } | undefined) => {
					...
					if (shouldSave) {
						...
						return {
							...old,
							clubs: [...old.clubs, club],
							totalSize: old.totalSize + 1,
						};
					}
					return {
						...old,
