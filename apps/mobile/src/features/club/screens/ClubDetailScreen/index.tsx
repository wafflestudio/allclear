...
const ClubDetailScreen = ({ route, navigation }: Props) => {
	...
	const [isVerificationOverlayVisible, setIsVerificationOverlayVisible] =
		useState(false);
	...
	const handlePreviousRecruitments = (
	...
	return (
		...
		>
			<SafeAreaView edges={["top", "left", "right"]} style={styles.safeArea}>
				...
				{club && (
					...
					>
						...
						<BackgroundCard style={styles.card}>
							<View style={styles.cardHeader}>
								<View style={styles.cardTitleRow}>
									...
									<VerificationMarkButton
										...
										onPress={() =>
											setIsVerificationOverlayVisible((prev) => !prev)
										}
									/>
								</View>
							...
							<VerificationOverlay
								...
								style={[
									...
									{
										transform: [
											{ translateY: -(verificationOverlayHeight + vs(8)) },
										],
									},
								]}
								onLayout={(event) =>
									setVerificationOverlayHeight(event.nativeEvent.layout.height)
								}
							/>
							{!!heroDescription && (
						...
						<View style={styles.tabContent}>
							{activeTab === "detail" && (
								<InfoTab club={club} contentWidth={HTML_CONTENT_WIDTH} />
							)}
				...
				{club && isTabBarPinned && (
					<View
						style={[styles.pinnedTabBar, { top: insets.top + HEADER_HEIGHT }]}
					>
						<ClubDetailTabBar activeKey={activeTab} onChange={setActiveTab} />
					</View>
				)}
				...
				>
					...
					<BackHeader
						title={isTabBarPinned ? (club?.name ?? "") : ""}
