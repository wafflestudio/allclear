...
import type { KakaoAccessTokenInfo } from "@react-native-seoul/kakao-login";
...
export type AuthRepository = {
	callbackKakaoAuth: (
		request: CallbackKakaoAuthRequest,
	) => Promise<CallbackKakaoAuthResponse>;
	callbackAppleAuth: (
		request: CallbackAppleAuthRequest,
	) => Promise<CallbackAppleAuthResponse>;
	leave: () => Promise<void>;
};
...
export const getAuthRepository = (): AuthRepository => ({
	...
	callbackAppleAuth: async (request: CallbackAppleAuthRequest) => {
		const response = await apiConnector.post<CallbackAppleAuthResponse>(
			"/v2/auth/apple/callback",
			{
